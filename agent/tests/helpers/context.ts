...
export const createRegistry = (page: Page, tabName: string): PageRegistry => ({
    ...
    cleanup: () => {},
    createWorkspace: async () => ({ workspaceName: 'ws-test', tabId: 'tab-test' }),
    ...
    resolveScope: () => ({ workspaceName: 'ws-test', tabId: 'tab-test' }),
    resolveTabBinding: () => ({ workspaceName: 'ws-test', tabId: 'tab-test' }),
