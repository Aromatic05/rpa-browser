...
let recordControlServices: RecordControlServices | null = null;

export const setRecordControlServices = (services: RecordControlServices): void => {
    recordControlServices = services;
};

const requireServices = (): RecordControlServices => {
    if (!recordControlServices) {
        throw new ActionError(ERROR_CODES.ERR_BAD_ARGS, 'record control services not initialized');
    }
    return recordControlServices;
};
...
export const handleRecordControlAction = async (input: WorkspaceControlInput): Promise<ControlPlaneResult> => {
