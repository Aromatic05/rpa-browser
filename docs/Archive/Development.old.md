...
- token 已存在于运行时，但 `token -> (workspaceName, tabName)` 尚未建立。
...
- 保存 `tabs`（`tabName/url/title/active`），不保存运行时 `tabName`。
- 保存录制 `steps`，并移除 step `meta.tabName`。
- 保存录制 `manifest` 的 tab 列表时，移除 `tabs[].tabName`。
- 多 tab step 持久化优先写 `args.tabRef`；运行时 `tabName` 与 `tabName` 不进入 core step YAML。
...
- 热回放会优先使用当前运行时的 `tabName -> tabName` 映射；仅在无法解析时才走 `browser.create_tab`（cold replay）
