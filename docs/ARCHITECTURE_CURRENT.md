...
- `sessionId/workspaceId`：对外可见的会话/工作区标识；目前 demo 使用 `workspace_manager.ts` 生成并仅在 demo 内存中维护。
- `tabId/groupId`：Session 内部 tab/group 概念；当前未在代码中成型，后续数据模型需预留字段。

## Current Runtime Model

- **绑定模型**：`tabToken` 由 content script 初始化并在 agent 端建立 `tabToken -> Page` 映射（证据见 `extension/src/content.ts` 与 `agent/src/runtime/page_registry.ts`）。
...
- **演进方向**：当前为 `tabToken -> Page` 绑定，未来将演进为 `session -> tabGroup -> tabs -> Page` 的多层模型（以实现为准）。
