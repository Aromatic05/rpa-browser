...
当 Control server 启动时注入了 `workspaceName` 上下文，router 会在请求缺失 `params.workspaceName` 时自动补入。若仍无法确定 workspace，方法会返回 `ERR_CONTROL_BAD_REQUEST`。

## dsl.run 语义

- 需要 `workspaceName` 与 `source`。
- 调用 `runDslSource`。
